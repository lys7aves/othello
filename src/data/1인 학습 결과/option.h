void SETUP()
{
	...
	remember[0] = 0;
	FILE *in_;
	in_ = fopen("user_name.txt","r");
	while(fscanf(in_, "%c", &user_name[k++])!=EOF);
	fclose(in_);
	sprintf(folder_name, "%s's data", user_name);
	mkdir(folder_name);
	sprintf(file_name, "%s\\%s's ai brain.txt", folder_name, user_name);
	FILE *in;
